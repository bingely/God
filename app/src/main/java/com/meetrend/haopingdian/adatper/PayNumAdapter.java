...
public class PayNumAdapter extends BaseAdapter{
	
	private List<InCommeBean> mList;
	private LayoutInflater mLayoutinInflater;
	
	public PayNumAdapter(List<InCommeBean> list,Context context){
		mList = list;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = mLayoutinInflater.inflate(R.layout.incomme_paynum_layout, null);//收入
		...
		if (NumerUtil.isFloat(inCommeBean.Payed + "") || NumerUtil.isInteger(inCommeBean.Payed+""))
			...
			payed = "0.00";
		viewHelper.inCommeTv.setText("收入：" + payed);
		...
		if (NumerUtil.isFloat(inCommeBean.AVG + "") || NumerUtil.isInteger(inCommeBean.AVG+""))
			...
			price = "0.00";
		viewHelper.priceTv.setText("客单价："+price);

		
		String pay = "支付笔数："+ (int)Math.floor(inCommeBean.Num);
		int contentLength = pay.length();
		SpannableStringBuilder style = new SpannableStringBuilder(pay);
		style.setSpan(new ForegroundColorSpan(Color.parseColor("#ff3b2d")), 5, 
				contentLength, Spannable.SPAN_EXCLUSIVE_INCLUSIVE); //设置指定位置文字的颜色    
		viewHelper.payNumTv.setText(style);
