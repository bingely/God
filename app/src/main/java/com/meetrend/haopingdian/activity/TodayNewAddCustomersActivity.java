...
import com.meetrend.haopingdian.adatper.TodayNewAddCustomersAdapter;
 ...
 *
 */
public class TodayNewAddCustomersActivity extends BaseActivity{
	...
	private TodayNewAddCustomersAdapter newAddCustomersAdapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		titleView.setText("今日新增客户");
		//当前日期
		curentTimeView.setText("当前时间：" + DateUtil.getCurrentDate());
		
		mList = new ArrayList<OrderBean>();
		newAddCustomersAdapter = new TodayNewAddCustomersAdapter(mList,TodayNewAddCustomersActivity.this);
		listview.setAdapter(newAddCustomersAdapter);
	...
	private void requestData(String pageSize,int pageIndex){
		...
		params.put("token", SPUtil.getDefault(TodayNewAddCustomersActivity.this).getToken());
		params.put("pageInedx", pageIndex + "");
		params.put("pageSize",pageSize);
		params.put("from","customerAdd");
		
		Callback callback = new Callback(tag, TodayNewAddCustomersActivity.this) {
			...
			public void onSuccess(String t) {
				...
				if (isSuccess) {
					...
					newAddCustomersAdapter.notifyDataSetChanged();
					...
					if (data.has("msg")) {
						...
						Toast.makeText(TodayNewAddCustomersActivity.this, msg, Toast.LENGTH_SHORT).show();
					}
				}
